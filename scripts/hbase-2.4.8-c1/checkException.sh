...
exceptions=$( grep -nr "Exception" $(find $monitor_path -name "*.log") | grep -v "ConnectException" | grep -v "ConnectionClosedException" | grep -v "ConnectTimeoutException" | grep -v "Failed get of master address: java.io.IOException" | grep -v "EOFException" | grep -v "at " | grep -v "CallTimeoutException" | grep -v "DelayQueue for RemoteProcedureDispatcher is not empty")
