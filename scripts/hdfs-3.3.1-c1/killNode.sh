function kill {
        ...
        case $1 in
           172.25.0.2)
              ...
              activeRst="$( docker exec -t C1HM1 /bin/bash -ic 'cd /home/gaoyu/evaluation/hb-hadoop-2.10.1/ && bin/yarn rmadmin -getServiceState rm1' )"
              rst=$(echo $activeRst| grep "active")
              echo "Goint to kill node C1HM1, active?: $rst"
              docker kill $nodeName
              sleep 30
              if [[ "$rst" != "" ]]; then
                activeRst="$( docker exec -t C1HM2 /bin/bash -ic 'cd /home/gaoyu/evaluation/hb-hadoop-2.10.1/ && bin/yarn rmadmin -getServiceState rm2' )"
                rst=$(echo $activeRst| grep "active")
                if [[ "$rst" == "" ]]; then
